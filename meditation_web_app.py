...
def process_meditation_request(scenario, duration, generate_audio_flag, session_id):
    ...
    try:
        ...
        # Store final result
        session[f"result_{session_id}"] = result
        
    except Exception as e:
        traceback.print_exc()
        update_progress(session_id, f"Error: {str(e)}", None)
        session[f"result_{session_id}"] = {"status": "error", "error": str(e)}
...
def progress(session_id):
    ...
    result_key = f"result_{session_id}"
    if result_key in session:
        updates.append({
            "message": "Generation complete!",
            "percentage": 100,
            "complete": True,
            "result": session[result_key]
        })
        # Clean up
        del session[result_key]
...
if __name__ == '__main__':
    print("="*60)
    print("🧘 AI Meditation Generator Starting...")
    print("="*60)
